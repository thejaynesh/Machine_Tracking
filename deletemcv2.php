<?php
    ...
    if(isset($_GET['mac_addr']) )
    {
        if ( strlen($_GET['mac_addr']) < 1 )
        {
            ...
            header('Location: deletemcv2.php');
            return;
        }
        else
        {
            $flag=0;
            $to;
            if(empty($_GET['mac_addr2']))
                $to=$_GET['mac_addr'];
            else
                $to=$_GET['mac_addr2'];
            for($i=$_GET['mac_addr'];$i<=$to;$i++)
            {
...
?></a>
    ...
    <form method="GET" action="deletemcv2.php" class="form-inline">

    <div class="input-group">
    <label for="mc no." class="required">&nbspMACHINE No. (from)&nbsp</label>
    <input type="text" name="mac_addr" class="form-control" required="" placeholder="Starting machine id" id="mcs" onchange="Number('mcs')">&nbsp </div>

    <div class="input-group">
    <label for="mc no." class="required">&nbspMACHINE No. (to)&nbsp</label>
    <input type="text" name="mac_addr2" class="form-control" placeholder="Ending machine id" id="mce" onchange="Number('mce')">&nbsp </div>
    ...
    <br><br>
            <h4>Delete Machine from list</h4>
    <center>
    ...
    <?php
        ...
        if($row['COUNT(*)']!=='0')
        {
            ...
            echo ("<table class=\"table table-striped\">
                <tr> <th>S.no.</th><th>MAC ADDRESS</th><th>Processor</th><th>RAM</th><th>Storage</th><th>OS</th><th>Keyboard</th><th>Mouse</th><th>Monitor</th><th>DOP</th><th>Price</th><th>Location</th> <th>State</th><th>DELETE</th></tr> ");
            while ( $row = $stmtread->fetch(PDO::FETCH_ASSOC) )
            {
                ...
                echo ("</td>"); 
                echo ("<td>");
                echo("<a href='deletemcv2.php?mac_addr=".htmlentities($row['MAC_ADDR'])."&mac_addr2=".htmlentities($row['MAC_ADDR'])."'><img src='img/delete.png' height='20' width='15'></a>");
                echo ("</td>");             
                $i++;
            }
            echo('</table>');
